...
function headstart_jetpack_setup() {
	...
	add_image_size( 'headstart-logo', 200, 200 );
	add_theme_support( 'site-logo', array( 'size' => 'headstart-logo' ) );
 ...
 */
function headstart_jetpack_default_hide_cf( $hidden, $screen ) {
...
add_action( 'default_hidden_meta_boxes', 'headstart_jetpack_default_hide_cf', 10, 2 );
 ...
 */
function headstart_the_site_logo() {
...
if( !function_exists( 'headstart_social_menu' ) ) :
 ...
 */
global $headstart_current_class;
$headstart_current_class = 'odd';
function headstart_odd_even_post_class( $classes ) {
    if( get_post_type( get_the_ID() ) === 'jetpack-testimonial' ) {
        global $headstart_current_class;
        $classes[] = $headstart_current_class;
        $headstart_current_class = ( $headstart_current_class == 'odd' ) ? 'even' : 'odd';
    }
    return $classes;
}
add_filter( 'post_class', 'headstart_odd_even_post_class' );

/**
 * Move JetPack Share and Like buttons
 * @link https://jetpack.com/2013/06/10/moving-sharing-icons/
 */
function headstart_jptweak_remove_share() {
...
add_action( 'loop_start', 'headstart_jptweak_remove_share' );