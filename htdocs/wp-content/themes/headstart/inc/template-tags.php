    ...
    //echo '<span class="byline"> ' . $byline . '</span>';
        ...
        /* translators: used between list items, there is a space after the comma */
                if( 'post' === get_post_type() ) {
                    $categories_list = get_the_category_list( __( '</li><li>', 'headstart' ) );
...
function headstart_entry_footer() {
    ...
    if ( 'post' === get_post_type() || 'jetpack-portfolio' === get_post_type() ) {
        ...
        $categories_list = get_the_category_list( esc_html__( ', ', 'headstart' ) );
                ...
                $categories_list = get_the_term_list( $post->ID, 'jetpack-portfolio-type', '', esc_html_x(', ', 'Used between list items, there is a space after the comma.', 'headstart' ), '');
            }
 ...
 */
function headstart_fancy_excerpt() {
    global $post;
    if( is_archive() ) {
       the_excerpt();
        echo '<div class="continue-reading">';
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_attr__( 'Continue Reading ', 'headstart' ) . get_the_title() . '" rel="bookmark">' . esc_html__( 'Continue Reading', 'headstart' ) . '</a>'; 
        echo '</div>';
    } elseif ( is_page_template( 'page-templates/page-child-pages.php' ) ) {
        the_excerpt();
         echo '<div class="continue-reading">';
        echo '<a class="continue-reading-arrow" href="' . esc_url( get_permalink() ) . '" title="' . esc_attr__( 'Continue Reading ', 'headstart' ) . get_the_title() . '" rel="bookmark">&rarr;</a>'; 
        echo '</div>';
    } elseif ( has_excerpt() || is_page_template( 'page-templates/frontpage-portfolio.php' ) ) {
        the_excerpt();
        echo '<div class="continue-reading">';
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_attr__( 'Continue Reading ', 'headstart' ) . get_the_title() . '" rel="bookmark">' . esc_attr__( 'Continue Reading', 'headstart' ) . '</a>'; 
        echo '</div>';
...
function headstart_author_box() {
    ...
    if ( isset( $post->post_author ) ) {
        ...
        $author_details .= '<h3 class="author-title"><span>' . esc_attr__( 'About ', 'headstart' );
            if ( is_author() ) $author_details .= $display_name;        // If an author archive, just show the author name
            else $author_details .= esc_html__( 'the Author', 'headstart' ); // If a regular page, show "About the Author"
        ...
        if ( ! is_author() ) {  // Don't show the meta info on an author archive page
            $author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_attr__( 'All posts by', 'headstart' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

            // Check if author has a website in their profile
            if ( ! empty( $user_site ) ) 
                $author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow">' . esc_attr__( 'Website', 'headstart' ) . '</a></p>';
            else $author_details .= '</p>';
        }
        
        $author_details .= '</section>';
        $author_details .= '</div>';
        $author_details .= '<p class="show-hide-author label">' . esc_attr__( 'Hide', 'headstart' ) . '</p>';
...
function headstart_portfolio_index_footer() {
    $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
    if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
            $time_string = '<i class="fa fa-refresh tip"><span class="tooltip">' .
                        esc_attr__( 'First posted: ', 'headstart' ) .
    ...
    $project_type = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta cat-links">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'headstart' ), '</span>' );
...
if ( ! function_exists( 'headstart_breadcrumbs' ) ) :
/**
 ...
 */
function headstart_breadcrumbs() {
    ...
    $home_link = '<a aria-label="' . esc_attr__( 'Home', 'headstart' ) .'" title="' . esc_attr__( 'Home', 'headstart' ) .'" class="breadcrumb-home" href="' . esc_url( home_url() ) . '"><i class="fa fa-home"></i></a>' . $separator;
    
    $output .= "<div aria-label='" . esc_attr__( 'You are here:', 'headstart' ) . "' id='$breadcrumb_id' class='$breadcrumb_class'>";
    $output .= $home_link;
 ...
 * Social Menu
 */
function headstart_social_menu() {
...
function headstart_the_post_icon() {
    echo wp_kses( headstart_post_icon(), array( 'span' => array( 'class' => array() ) ) );
}

/**
 * Function to show the Jetpack sharing and Likes only at the designated locations in Posts and Pages
 */
function headstart_jetpack_sharing() {
 ...
 */
function headstart_post_navigation() {
    ...
    <nav class="navigation post-navigation" role="navigation">
        <p class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'headstart' ); ?></p>
        <div class="nav-links" data-equalizer>
                <?php
                        previous_post_link( '<div class="nav-previous" data-equalizer-watch><div class="nav-indicator">' . esc_html_x( 'Previous Post:', 'Previous post', 'headstart' ) . '</div><h4>%link</h4></div>', '%title' );
                        next_post_link(     '<div class="nav-next" data-equalizer-watch><div class="nav-indicator">'     . esc_html_x( 'Next Post:', 'Next post', 'headstart' ) . '</div><h4>%link</h4></div>', '%title' );
                ?>
        </div> <!-- .nav-links -->
    </nav> <!-- .navigation -->
    <?php
...
if ( ! function_exists( 'headstart_paging_nav' ) ) :
/**
 ...
 */
function headstart_paging_nav() {
        the_posts_pagination( array(               
            'mid_size'      => 3,
            'prev_text' => __( 'Previous', 'headstart' ),
            'next_text' => __( 'Next', 'headstart' ),
            'type'      => 'list',
        ));
}
endif;
...
if ( ! function_exists( 'headstart_copyright' ) ) :
/** 
 ...
 */
function headstart_front_page_section( $partial = null, $page_name = "" ) {
    ...
    if ( get_theme_mod( 'panel_' . $page_name ) ) {
        ...
        echo '<article class="panel-placeholder panel headstart-panel headstart-panel-' . $page_name . '" id="panel-' . $page_name . '"><span class="headstart-panel-title">' . sprintf( __( '%1$s Page Placeholder', 'headstart' ), $page_name ) . '</span></article>';
    }
}